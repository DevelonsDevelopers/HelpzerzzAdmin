...
const CategoryAddEdit = ({ edit = false }) => {
    ...
    const [categoryData, setCategoryData] = useState({name: '', details: '', image: ''})
    ...
    return (
        <PortalLayout>
            {response.categoryLoading ?
                ...
                :
                <div>
                    <h1 className='text-center text-[25px] font-[800] mt-5 uppercase'>{edit ? "edit category" : "add category"}</h1>
                    <div
                        className="bg-white mt-[3rem] rounded-xl px-[8rem] py-16 flex flex-col mx-8">
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <div className="w-[100%] px-5 py-2">
                                    <label className="block text-[12px] ml-3 font-medium uppercase">Name</label>
                                    <input value={categoryData.name} onChange={(e) => handleChange(e)} type="text"
                                           name={names[0]} placeholder="Enter Your Name"
                                           className={`pl-4 block py-[9px] w-full text-sm bg-gray-50 rounded-[9px] border-[1px] focus:border-black focus:outline-none mt-1 ${errors[0] ? "border-red-600" : "border-gray-300"}`}/>
                                </div>
                                <div className="w-[100%] px-5 py-2">
                                    <label className="block text-[12px] ml-3 font-medium uppercase">Details</label>
                                    <textarea value={categoryData.details} onChange={(e) => handleChange(e)}
                                              name={names[1]} minLength='30' maxLength='500' rows='7'
                                              placeholder="Enter details here"
                                              className={`pl-4 block py-[9px] w-full text-sm bg-gray-50 rounded-[9px] border-[1px] focus:border-black focus:outline-none mt-1 ${errors[1] ? "border-red-600" : "border-gray-300"}`}/>
                                </div>
                            </div>
                            <div>
                                <div className="w-[100%] ">
                                    <label htmlFor="dropzone-file"
                                        ...
                                        <input type='file' id="dropzone-file" name={names[2]} className='hidden'
