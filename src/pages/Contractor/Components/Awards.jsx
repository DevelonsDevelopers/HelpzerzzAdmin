...
import {addAward, awardSuccessListener} from "../../../api/reducers/contractor";
...
const Awards = ({id, response}) => {
    ...
    useEffect(() => {
        if (id){
            setAwardData((data) => ({...data, contractor: parseInt(id)}))
        }
    }, [id]);

    useEffect(() => {
        if (response.awardSuccess) {
            navigate('/contractors')
            dispatch(awardSuccessListener())
        }
    }, [response.awardSuccess]);
    ...
    const convertToBase64 = (e) => {
        ...
        if (e.target.files.length > 0) {
            ...
            reader.onload = () => {
                ...
                setAwardData((data) => ({ ...data, image: reader.result }));
            };
        }
    };
    ...
    const handleChange = (e) => {
        ...
        setErrors(tempErrors);
        setAwardData((data) => ({ ...data, [e.target.name]: e.target.value }));
    };
    ...
    const handleSubmit = () => {
        ...
        for (let i = 0; i < names.length; i++) {
            let name = names[i];
            tempErrors[i] = awardData[name].length === 0;
        }
        setErrors(tempErrors);
            ...
            {add ?
                                    ...
                                    {awardData.image ? (
                                        <>
                                            {file ? (
                                                ...
                                                <img
                                                    src={`${IMAGE_PATH}${awardData.image}`}
                                                    alt=""
                                                    className={`rounded-lg object-contain h-56 w-full py-[2px]`}
                                                />
                                            )}
                ...
                <>
                    ...
                    <div className={`grid grid-cols-5 gap-2 mt-6`}>
                        {response?.contractorDetails?.awards?.map(value => (
                            <div
